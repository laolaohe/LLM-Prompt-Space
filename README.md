...
1. 输入层：结构化隔离 
动态 XML 封装 (xml_wrap_messages)：使用随机生成的标签名封装用户输入，并进行字符转义，防止攻击者通过伪造闭合标签（如 </user>）逃逸。

随机令牌边界 (random_wrapper)：利用高熵随机字符串构建物理隔离带，使攻击者无法预测定界符。

2. 执行层：语义锚定与过滤 
关键词过滤 (sanitize_messages)：针对中文语境优化的正则表达式过滤，拦截高危系统命令、角色扮演词汇及绕过指令。

三明治防御 (sandwich_defense)：采用首尾夹击策略，利用模型的“近因效应（Recency Bias）”强化安全准则。

绝对化后提示 (add_post_prompt)：在上下文末端植入锁定指令，对抗长文本场景下的注意力偏移。

3. 输出层：实时语义审计 
强化审计模块 (llm_evaluate_response)：引入独立的“审计者模型”，对 LLM 的输出进行二次校验。通过结构化隔离保护审计者不被二次劫持，严防密钥泄露与违规代码输出。
